...
final class SummitAttendeeSerializer extends SilverStripeSerializer
{
    ...
    public function serialize($expand = null, array $fields = array(), array $relations = array(), array $params = array())
    {
        ...
        if($attendee->hasMember())
        {
            ...
            $values['bio']        = JsonUtils::toJsonString($member->getGender());
                ...
                switch (trim($relation)) {
                    case 'schedule': {
                        ...
                        foreach ($attendee->getSchedule() as $s) {
                            array_push($schedule, SerializerRegistry::getInstance()->getSerializer($s)->serialize($expand, $fields, $relations, $params));
                        }
                    ...
                    case 'ticket_type': {
                        ...
                        foreach($attendee->getTickets() as $t)
                        {
                            array_push($tickets, SerializerRegistry::getInstance()->getSerializer($t->getTicketType())->serialize($expand, $fields, $relations, $params));
                        }
                        $values['tickets'] = $tickets;
                    }
                    break;
                    case 'speaker': {
                        if (!is_null($speaker))
                        {
                            ...
                            $values['speaker'] = SerializerRegistry::getInstance()->getSerializer($speaker)->serialize($expand, $fields, $relations, $params);
                        }
                    }
                    break;
                    case 'feedback': {
                        ...
                        foreach ($attendee->getEmittedFeedback() as $f) {
                            array_push($feedback,  SerializerRegistry::getInstance()->getSerializer($f)->serialize($expand, $fields, $relations, $params));
                        }
