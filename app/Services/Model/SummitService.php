        ...
        try {
            $this->tx_service->transaction(function () use ($summit, $member, $event_id, $check_rsvp) {
                ...
                if($member->hasSyncInfoFor($summit)) {
                    ...
                    $pending_request = $this->calendar_sync_work_request_repository->getUnprocessedMemberScheduleWorkRequest
                    (
                        $member,
                        $event,
                        $sync_info
                    ...
                    if(!is_null($pending_request)) {
                        // if we have a pending job... skip processing
                        if($member->isEventSynchronized($event) && $pending_request->getType() == AbstractCalendarSyncWorkRequest::TypeRemove)
                            // if pending job is remove and event is already sync, delete former and skip it
                            $this->calendar_sync_work_request_repository->delete($pending_request);
                    }
                    else {
    ...
    public function removeEventFromMemberSchedule(Summit $summit, Member $member, $event_id, $check_rsvp = true)
    {
        $this->tx_service->transaction(function () use ($summit, $member, $event_id, $check_rsvp) {
            ...
            if($member->hasSyncInfoFor($summit)) {
                ...
                if(!is_null($pending_request))
                    // delete former pending insertion job
                    $this->calendar_sync_work_request_repository->delete($pending_request);

                // is event had already calendar sync emit delete request
                if($member->isEventSynchronized($event)) {
                    $request = new MemberEventScheduleSummitActionSyncWorkRequest();
