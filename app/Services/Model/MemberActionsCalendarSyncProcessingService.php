                    ...
                    switch ($request_sub_type) {
                        ...
                        case MemberEventScheduleSummitActionSyncWorkRequest::SubType: {
                            ...
                            switch ($request_type) {
                                case AbstractCalendarSyncWorkRequest::TypeAdd:
                                    try {
                                        ...
                                        if ($calendar_sync_info->isRevoked()){
                                            Log::warn(sprintf("EVENT ADD : event id %s - member id %s could not be added on external calendar bc credential are revoked!", $summit_event->getId(), $member->getId()));
                                            continue;
                                        }
                                        ...
                                        if(is_null($schedule_sync_info)){
                                            Log::warn(sprintf("EVENT ADD : event id %s - member id %s could not be added on external calendar", $summit_event->getId(), $member->getId()));
                                            continue;
                                        }
                                ...
                                case AbstractCalendarSyncWorkRequest::TypeRemove:
                                    ...
                                    if(is_null($schedule_sync_info)){
                                        Log::warn(sprintf("EVENT REMOVE : event id %s - member id %s could not be removed, schedule synch info is null", $summit_event->getId(), $member->getId()));
                                        continue;
                                    }
                        ...
                        case MemberCalendarScheduleSummitActionSyncWorkRequest::SubType: {
                            switch ($request_type) {
                                case AbstractCalendarSyncWorkRequest::TypeAdd:
                                    if($calendar_sync_info->isRevoked()){
                                        Log::warn(sprintf("CALENDAR ADD : calendar sync info id %s, member id %s is revoked!", $calendar_sync_info->getId(), $member->getId()));
                                        continue;
                                    }
