...
implements IMemberActionsCalendarSyncProcessingService
{
    ...
    public function processActions($provider = 'ALL', $batch_size = 1000)
    {
        return $this->tx_manager->transaction(function() use($provider, $batch_size){
            ...
            log::info(sprintf("got %s request to process ...", count($requests)));
            ...
            foreach ($requests as $request){
                try {
                    ...
                    log::info(sprintf
                    (
                        "processing work request %s - sub type %s - type %s - member %s - credential id %s -revoked credentials %s",
                    ...
                    switch ($request_sub_type) {
                        ...
                        case MemberEventScheduleSummitActionSyncWorkRequest::SubType: {
                            ...
                            log::info(sprintf
                            (
                                "processing work request %s - sub type %s - type %s - event id %s - member %s - credential id %s -revoked credentials %s",
                                $request->getIdentifier(),
                                $request_sub_type,
                                $request_type,
