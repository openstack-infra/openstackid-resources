...
final class ChatTeamService implements IChatTeamService
{
    ...
    {
        $this->tx_service->transaction(function() use($batch_size){

           $teams_ids = $this->repository->getAllTeamsIds();

           foreach($teams_ids as $team_id) {

               $messages = $this->chat_message_repository->getAllSentByTeamPaginated
               (
                   $team_id,
               ...
               foreach ($messages as $message){
                   ...
                   $data  = [
                       ...
                       'from'       => [
                           'id'         => intval($message->getOwner()->getId()),
                           'first_name' => $message->getOwner()->getFirstName(),
                           'last_name'  => $message->getOwner()->getLastName(),
                       ],
