...
final class SummitService implements ISummitService
{
    ...
    public function addEvent(Summit $summit, array $data)
    {
        return $this->tx_service->transaction(function() use($summit, $data) {
            ...
            $start_datetime  = intval($data['start_datetime']);
            $start_datetime  = new \DateTime("@$start_datetime");
            $end_datetime    = intval($data['end_datetime']);
            ...
            if(is_null($event_type))
                throw new ValidationException(sprintf("event type id %s does not exists!", $data['type_id']));
            $location        = $summit->getLocation(intval($data['location_id']));
            if(is_null($location))
                throw new ValidationException(sprintf("location id %s does not exists!", $data['location_id']));
            $summit_types    = array();
            ...
            $event = SummitEventFactory::build
            (
                $data['title'],
                $data['description'],
                $start_datetime,
            ...
            if(!$summit->isEventInsideSummitDuration($event))
                ...
                (
                    ...
                    (
                        ...
                        $summit->SummitBeginDate,
                        $summit->SummitEndDate
                    )
                );
