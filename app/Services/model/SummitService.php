...
use App\Events\AddSummitEventFeedback;
use App\Events\MyScheduleAdd;
use App\Events\MyScheduleRemove;
use App\Events\SummitEventPublished;
use App\Events\SummitEventUnPublished;
...
final class SummitService implements ISummitService
{
    ...
    public function getSummitEntityEvents(Summit $summit, $member_id = null, \DateTime $from_date = null, $from_id = null)
    {
        ...
        Log::debug(sprintf('getSummitEntityEvents - $member_id %s - $from_date %s - $from_id %s', $member_id, $from_date, $from_id));
        ...
        foreach($events as $e)
        {
            ...
            switch($e->EntityClassName)
            {
                ...
                case 'MySchedule':
                {
                    Log::debug(sprintf('MySchedule - $member_id %s - $e->OwnerID %s', $member_id,$e->OwnerID));
                ...
                case 'WipeData':
                {
                    ...
                    if(intval($e->EntityID) > 0 )
                    {
                        ...
                        if( intval($member_id) !== intval($e->EntityID) || $e->isProcessed())
                            continue;
                        $e->markProcessed()->save();
                    }
