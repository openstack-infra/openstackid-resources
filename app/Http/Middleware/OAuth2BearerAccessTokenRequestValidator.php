...
class OAuth2BearerAccessTokenRequestValidator implements Middleware {
    ...
    public function handle($request, Closure $next)
    {
        try {
            ...
            //check lifetime
            if($token_info->getLifetime() <= 0){
                throw new OAuth2ResourceServerException(400, OAuth2Protocol::OAuth2Protocol_Error_InvalidRequest, 'invalid access token');
            }
            ...
            if($token_info->getClientType() === 'JS_CLIENT' && strstr($origin, $token_info->getAllowedOrigins()) === false){
                //check origins
