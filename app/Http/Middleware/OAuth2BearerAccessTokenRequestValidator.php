...
class OAuth2BearerAccessTokenRequestValidator implements Middleware {
     ...
     */
    public function __construct(IResourceServerContext $context, IApiEndpointRepository $endpoint_repository, IAccessTokenService $token_service){
    ...
    public function handle($request, Closure $next)
    {
        ...
        try {
            ...
            if(is_null($endpoint) || !$endpoint->isActive()){
                throw new OAuth2ResourceServerException(400,OAuth2Protocol::OAuth2Protocol_Error_InvalidRequest,sprintf('API endpoint does not exits! (%s:%s)', $route, $method));
            }
            ...
            if(is_null($token_info) || $token_info->getLifetime() <= 0){
                throw new OAuth2ResourceServerException(401,OAuth2Protocol::OAuth2Protocol_Error_UnauthorizedClient, 'invalid origin');
            }
            ...
            if((!in_array($realm, $audience)))
                throw new OAuth2ResourceServerException(401,OAuth2Protocol::OAuth2Protocol_Error_InvalidToken,'the access token provided is expired, revoked, malformed, or invalid for other reasons.');

            if($token_info->getApplicationType() === 'JS_CLIENT' && strstr($origin, $token_info->getAllowedOrigins()) === false){
                //check origins
            ...
            {
                Log::error(sprintf('access token scopes (%s) does not allow to access to api url %s , needed scopes %s',$token_info->getScope(),$url,implode(' OR ',$endpoint_scopes) ));
