...
final class RateLimitMiddleware implements Middleware {
    ...
    public function handle($request, Closure $next)
    {
        ...
        if($response->getStatusCode() === 304) return $response;
        ...
        try {
            ...
            if(!is_null($endpoint->rate_limit) && ($requestsPerHour = (int)$endpoint->rate_limit) > 0){
                ...
                if( $count > $requestsPerHour )
                {
                    ...
                    $response = Response::json(array('message' => "You have triggered an abuse detection mechanism and have been temporarily blocked. Please retry your request again later."), 403);
