    ...
    public function getAll($summit_id)
    {
        try
        {
            ...
            if (Input::has('filter')) {
                ...
                $channels = $filter->getFilter("channel");
                if(!is_null($channels)){
                    foreach ($channels as $element){
                        if(is_array($element)){
                            foreach ($element as $e){
                                if(!SummitPushNotificationChannel::isPublicChannel($e->getValue()))
                                    throw new ValidationException(sprintf(" %s channel is not public!", $e->getValue()));
                            }
                        }
                        else if(!SummitPushNotificationChannel::isPublicChannel($element->getValue()))
