...
class OAuth2SummitApiController extends OAuth2ProtectedController
{
                ...
                foreach ($expand as $relation) {
                    switch (trim($relation)) {
                        ...
                        case 'schedule': {
                            ...
                            $speakers = array();
                            list($total, $per_page, $current_page, $last_page, $items) = $summit->speakers(1,
                                PHP_INT_MAX);
    ...
    public function getAttendees($summit_id)
    {
        try {
            ...
            list($total, $per_page, $current_page, $last_page, $items) = $summit->attendees($page, $per_page, $filter);
    ...
    public function getSpeakers($summit_id)
    {
        try {
            ...
            list($total, $per_page, $current_page, $last_page, $items) = $summit->speakers($page, $per_page, $filter);
    ...
    public function getEvents($summit_id)
    {
        try {
            ...
            $filter = null;
            if (Input::has('filter')) {
                $filter = FilterParser::parse(Input::has('get'), array
                (
                    'title'      => array('=@', '=='),
                    'start_date' => array('>', '<', '<=', '>=', '=='),
                    'end_date'   => array('>', '<', '<=', '>=', '=='),
