...
class OAuth2SummitApiController extends OAuth2ProtectedController
{
            ...
            if (!empty($expand)) {
                ...
                foreach ($expand as $relation) {
                    switch (trim($relation)) {
                        case 'attendees': {
                            $attendees = array();
                            list($total, $per_page, $current_page, $last_page, $items) = $summit->attendees(1,999999);
                        ...
                        case 'schedule': {
                            ...
                            $speakers = array();
                            list($total, $per_page, $current_page, $last_page, $items) = $summit->speakers(1,999999);
    ...
    public function getSummitEntityEvents($summit_id)
    {
        try {
            ...
            if(!is_null($from_date)) $from_date = new \DateTime($from_date);
    ...
    public function getAttendees($summit_id)
    {
        try {
            ...
            list($total,$per_page, $current_page, $last_page, $items) = $summit->attendees($page, $per_page);
                ...
                foreach ($expand as $relation) {
                    switch (trim($relation)) {
                        ...
                        case 'speaker':
                        {
                            ...
                            if(!is_null($speaker))
                            {
                                $speaker_data = $speaker->toArray();
                                $speaker_data['speaker_id'] = $speaker_data['id'];
                                unset($speaker_data['id']);
    ...
    public function getSpeakers($summit_id)
    {
        try {
            ...
            list($total,$per_page, $current_page, $last_page, $items) = $summit->speakers($page, $per_page);
     ...
     * @return mixed
     */
    public function addSpeakerFeedback(LaravelRequest $request, $summit_id, $speaker_id, $presentation_id)
    {
        if (!$request->isJson()) {
        ...
        try {
            ...
            if( strtolower($speaker_id) === 'all' || intval($speaker_id) > 0 )
            {
                $presentation = $summit->getScheduleEvent(intval($presentation_id));
            ...
            if(intval($speaker_id) > 0 )
            {
                ...
                $presentation = $speaker->getPresentation(intval($presentation_id));
            ...
            if(is_null($speaker))
            {
                $res = $this->service->addSpeakerFeedbackAll
                (
                    $summit,
                    $presentation,
                    $data->all()
                ...
                return !is_null($res) ? $this->created($ids) : $this->error400();
            }
            else
            {
