...
class OAuth2SummitApiController extends OAuth2ProtectedController
{
    ...
    public function addEventToAttendeeSchedule($summit_id, $attendee_id, $event_id)
    {
        try {
            ...
            if ($attendee_id === 'me') {
                ...
                $attendee  = $summit->getAttendeeByMemberId($member_id);
                if(!is_null($attendee)) $attendee_id = intval($attendee->ID);
            } else {
                $attendee = $summit->getAttendeeById(intval($attendee_id));
            }
            ...
            if(intval($attendee->ID) !== intval($attendee_id)){
    ...
    public function getEventFeedback($summit_id, $event_id, $attendee_id = null)
    {
        ...
        try {
            ...
            list($total, $per_page, $current_page, $last_page, $feedback) = $event->feedback($page, $per_page, $filter);
    ...
    public function addEventFeedback(LaravelRequest $request, $summit_id, $event_id)
    {
        try {
            ...
            if ($attendee_id === 'me') {
                ...
                $attendee = $summit->getAttendeeByMemberId($member_id);
                if(!is_null($attendee))
                    $attendee_id = $data['attendee_id'] = intval($attendee->ID);
            }
            else
            {
                $attendee = $summit->getAttendeeById(intval($attendee_id));
