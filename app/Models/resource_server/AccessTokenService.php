...
final class AccessTokenService implements IAccessTokenService
{
    ...
    public function get($token_value)
    {
        ...
        if (count($token_info) === 0)
        {
            ...
            $this->cache_service->storeHash(md5($token_value), $token_info, (int) Config::get('server.access_token_cache_lifetime', 300));
        }
        else
        {
            $cache_lifetime = intval($this->cache_service->ttl(md5($token_value)));
            $token_info['expires_in'] = intval($token_info['expires_in']) - $cache_lifetime;
            Log::debug(sprintf("token life time %s - token cache lifetime %s", $token_info['expires_in'], $cache_lifetime));
        }
