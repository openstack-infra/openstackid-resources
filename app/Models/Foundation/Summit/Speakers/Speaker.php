...
use App\Models\Foundation\Summit\Events\Presentations\PresentationSpeaker;
...
class Speaker extends SilverstripeBaseModel
{
    ...
    public function getPresentationsBySelectionPlanAndRole(SelectionPlan $selectionPlan, $role){
        ...
        if($role == self::ROLE_SPEAKER){
            $res = $this->presentations->filter(function(PresentationSpeaker $presentation_speaker) use($selectionPlan){
                if($presentation_speaker->getPresentation()->getSelectionPlanId() != $selectionPlan->getId()) return false;
                if($presentation_speaker->getPresentation()->getSummit()->getId() != $selectionPlan->getSummitId()) return false;
                if($presentation_speaker->getSpeaker() == $this->getId() && $presentation_speaker->getRole() == self::RoleModerator) return false;
                if($presentation_speaker->getPresentation()->getCreatorId() == $this->getMemberId()) return false;
            });
            return $res->toArray();
        }
