...
class SummitAttendee extends SilverstripeBaseModel
{
    ...
    public function emitted_feedback(){
        $res = SummitEventFeedback::where('OwnerID', '=', $this->MemberID)->orderBy('ID','asc')->get();
        $feedback = array();
        foreach($res as $e)
        {
            $class = 'models\\summit\\'.$e->ClassName;
    ...
    public function toArray()
    {
        ...
        if(!is_null($member))
        {
            ...
            $values['pic']        = ($member->photo() !== null) ? Config::get("server.assets_base_url", 'https://www.openstack.org/'). $member->photo()->Filename : null;
